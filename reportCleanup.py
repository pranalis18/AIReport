...
if slideInfo:
    ...
    with open('metadata.cart.2023-07-24.json') as file:
        slideInfo = json.load(file)

    matching_entry = next((entry for entry in slideInfo if entry['file_name'] == slideName), None)
    caseID = matching_entry['associated_entities'][0]['case_id']
    caseInfo = {
        "ID": clinical.at[caseID, 'caseID'],
        "Gender": clinical.at[caseID, 'gender'],
        "Age": clinical.at[caseID, 'Age'],
        "ER": clinical.at[caseID, 'ER.Status'],
        "PR": clinical.at[caseID, 'PR.Status'],
        "Her2": clinical.at[caseID, 'HER2.Final.Status'],
        "Stage": clinical.at[caseID, 'AJCC.Stage'],
        "PAM50": clinical.at[caseID, 'PAM50.mRNA'],
        "link": "https://portal.gdc.cancer.gov/cases/" + caseID
    }

    #slide information JSON file
